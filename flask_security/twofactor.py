...
def send_security_token(user, method, token):
    ...
    """
    token_to_be_sent = get_totp_password(token)
    if method == 'mail':
...
def get_totp_uri(user):
    ...
    :return:
    """
    return 'otpauth://totp/emedgene:{0}?secret={1}&issuer=emedgene'.format(user.username, user.primary_totp)

def verify_totp(user_totp, token, window=0):
