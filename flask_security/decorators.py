...
def _check_token():
    ...
    if user and callable(user.is_authenticated) and user.is_authenticated() or user.is_authenticated:
...
def auth_required(*auth_methods):
    ...
    login_mechanisms = {
        ...
        'session': lambda: callable(current_user.is_authenticated) and current_user.is_authenticated() or current_user.is_authenticated
    }
